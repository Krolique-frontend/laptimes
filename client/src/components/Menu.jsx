...
const Menu = ({raceDay, toMain}) => {
  // const dates = props.dates;
  const {request} = useHttp();
  const alertDiv = useRef(null);
  const m = useRef(null);
  const [todayPage, setTodayPage] = useState(false);
  const setToday = useCallback(async () => {
    try {
      const pageUrl = window.location.href;
      if (pageUrl.includes('today')) setTodayPage(true);
    } catch (e) {
      console.log(JSON.stringify(e));
    }
  }, [request]);

  useEffect(() => setToday(), [setToday]);
      ...
      <ul className={menu.menu} ref={m}>
        <li className={menu.menuItem}>
          <button
            ...
            style={todayPage ? {display: 'none'} : null}
          >
            Трэк
          </button>
        ...
        <li className={menu.menuItem}>
          <a
            ...
            href={todayPage ? '/' : '/today'}
            onClick={todayHandler}
          >
            {todayPage ? 'на главную' : 'гонка'}
          </a>
        </li>
